        ...
        <div id="stages-view" class="transition-view">
            <div class="d-flex align-items-center mb-4">
                ...
                <div>
                    ...
                    <p class="text-muted mb-0 small">Chọn công đoạn để bắt đầu cập nhật dữ liệu và kiểm tra trạng thái phòng</p>
                </div>
            </div>
            ...
            <div class="mb-4 d-flex justify-content-between align-items-center flex-wrap">
                <div class="btn-group shadow-sm mb-2 mb-md-0" role="group" aria-label="Workshop Selection">
                    @foreach($workshopsList as $ws)
                        <button type="button" class="btn btn-outline-primary workshop-btn {{ $loop->first ? 'active' : '' }}" 
                ...
                <div class="d-flex align-items-center gap-3 small bg-white px-3 py-2 rounded-pill shadow-sm">
                    ...
                    <span class="text-secondary"><i class="fas fa-circle text-success ms-1"></i> Đang sản xuất</span>
            ...
            @foreach($stageProductions as $workshop => $stages)
                <div class="row g-4 mt-2 workshop-stages-container" id="workshop-stages-{{ $workshop }}" {!! $loop->first ? '' : 'style="display: none;"' !!}>
                            ...
                            <div class="stage-card h-100 border-0 shadow-sm rounded-4 overflow-hidden position-relative" 
                                ...
                                <div class="card-body p-4 d-flex flex-column justify-content-between position-relative z-index-1">
                                    <div class="d-flex justify-content-between align-items-start mb-4">
                                        <div class="stage-icon bg-white text-dark rounded-3 p-3 shadow-sm {{ str_contains($stage->icon_class, 'notch') ? 'animate-spin-slow' : '' }}">
                                    ...
                                    <div>
                                        <h4 class="fw-bold text-white mb-2 text-uppercase">{{ $stage->stage_name }}</h4>
                                        <div class="d-flex flex-wrap gap-1 mt-2">
                                            @foreach($rooms as $room)
                                                @if($room->deparment_code == $workshop && in_array($room->stage_code, $stage->stage_codes))
                                                    ...
                                                    @endphp
                                                    <span class="badge bg-white text-dark shadow-sm" title="{{ $sTitle }}" style="opacity: 0.95;">
                                                        {{ $room->code }}
                                                        <i class="fas fa-circle {{ $sColor }} ms-1" style="font-size: 0.55rem; vertical-align: middle;"></i>
                                                    </span>
                                                @endif
                                            @endforeach
                                        </div>
        ...
        <div id="rooms-view" class="transition-view" style="display: none;">
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-4 pb-3 border-bottom">
                <div class="d-flex align-items-center mb-3 mb-md-0">
                    <button class="btn btn-outline-secondary rounded-pill px-3 py-2 shadow-sm me-3 bg-white" onclick="showStagesView()">
                    ...
                    <div>
                        <h3 class="mb-0 text-navy fw-bold d-flex align-items-center">
                            ...
                            CÔNG ĐOẠN: <span id="active-stage-title" class="ms-2 text-primary">Cân Nguyên Liệu</span>
                        </h3>
                        <p class="text-muted mb-0 small">Khai báo và theo dõi nhật ký hoạt động của các phòng sản xuất</p>
                    </div>
                </div>
                <div class="badge bg-navy rounded-pill px-4 py-2 shadow-sm fs-6 text-white" id="rooms-count-badge">0 Phòng</div>
            </div>
            ...
            <div class="row g-4" id="rooms-grid">
                @forelse($rooms as $room)
                    @php
                        ...
                        $time = time();
                        $mockTemp = number_format((20.0 + ($room->id % 5) * 0.8) + sin($time / 30.0 + $room->id) * 0.4, 1);
                        $mockHumid = number_format((42 + ($room->id % 6) * 1.5) + cos($time / 45.0 + $room->id) * 2, 0);
                        $mockPressure = '+' . number_format((8 + ($room->id % 8)) + sin($time / 60.0 + $room->id) * 1, 0);
                    ...
                    @endphp
                    <div class="col-12 col-md-6 col-lg-4 room-card-col" data-stage-code="{{ $room->stage_code }}" data-workshop-code="{{ $room->deparment_code }}">
                        <div class="card h-100 border-0 shadow-sm rounded-4 overflow-hidden room-card transition-all">
                            <div class="card-header bg-white pt-4 px-4 border-0 d-flex justify-content-between align-items-center">
                                <div>
                                    <span class="badge bg-navy text-white rounded-pill px-3 py-1 font-monospace mb-1">{{ $room->code }}</span>
                                    <h5 class="fw-bold text-navy mb-0 mt-1">{{ $room->name }}</h5>
                                </div>
                                <div class="d-flex flex-column align-items-end gap-1">
                                    <span class="status-indicator {{ $statusClass }} px-3 py-1 rounded-pill fw-bold text-uppercase" style="font-size: 0.75rem;">
                                        {{ $statusText }}
                                    </span>
                                </div>
                            </div>
                            <div class="card-body px-4 pb-4 pt-2">
                                ...
                                <div class="telemetry-panel rounded-3 p-3 bg-light d-flex justify-content-between mb-3 shadow-inner">
                                    <div class="text-center flex-fill border-end px-1">
                                        <div class="small text-muted mb-1 font-monospace d-flex justify-content-center align-items-center" style="font-size: 0.65rem;">
                                            <span>NHIỆT ĐỘ</span>
                                            <span class="badge bg-danger text-white px-1 py-0 ms-1 rounded warning-badge d-none" id="warning-temp-{{ $room->id }}" style="font-size: 0.55rem;">CẢNH BÁO</span>
                                        </div>
                                        <div class="fw-bold text-navy mb-1" style="font-size: 1.1rem;"><span class="temp-val" id="room-temp-{{ $room->id }}">{{ $mockTemp }}</span><span style="font-size: 0.8rem;">°C</span></div>
                                        <canvas id="chart-temp-{{ $room->id }}" class="sparkline-canvas" width="200" height="80" 
                                                data-room-id="{{ $room->id }}" data-metric="temp"
                                                data-min="{{ $room->limits['temp_min'] }}" data-max="{{ $room->limits['temp_max'] }}"></canvas>
                                        <div class="text-muted font-monospace mt-1" style="font-size: 0.55rem; opacity: 0.8;">
                                    ...
                                    <div class="text-center flex-fill border-end px-1">
                                        <div class="small text-muted mb-1 font-monospace d-flex justify-content-center align-items-center" style="font-size: 0.65rem;">
                                            <span>ĐỘ ẨM</span>
                                            <span class="badge bg-danger text-white px-1 py-0 ms-1 rounded warning-badge d-none" id="warning-humid-{{ $room->id }}" style="font-size: 0.55rem;">CẢNH BÁO</span>
                                        </div>
                                        <div class="fw-bold text-navy mb-1" style="font-size: 1.1rem;"><span class="humid-val" id="room-humid-{{ $room->id }}">{{ $mockHumid }}</span><span style="font-size: 0.8rem;">%</span></div>
                                        <canvas id="chart-humid-{{ $room->id }}" class="sparkline-canvas" width="200" height="80" 
                                                data-room-id="{{ $room->id }}" data-metric="humid"
                                                data-min="{{ $room->limits['humid_min'] }}" data-max="{{ $room->limits['humid_max'] }}"></canvas>
                                        <div class="text-muted font-monospace mt-1" style="font-size: 0.55rem; opacity: 0.8;">
                                    ...
                                    <div class="text-center flex-fill px-1">
                                        <div class="small text-muted mb-1 font-monospace d-flex justify-content-center align-items-center" style="font-size: 0.65rem;">
                                            <span>ÁP SUẤT</span>
                                            <span class="badge bg-danger text-white px-1 py-0 ms-1 rounded warning-badge d-none" id="warning-pressure-{{ $room->id }}" style="font-size: 0.55rem;">CẢNH BÁO</span>
                                        </div>
                                        <div class="fw-bold text-navy mb-1" style="font-size: 1.1rem;"><span class="pressure-val" id="room-pressure-{{ $room->id }}">{{ $mockPressure }}</span><span style="font-size: 0.8rem;">Pa</span></div>
                                        <canvas id="chart-pressure-{{ $room->id }}" class="sparkline-canvas" width="200" height="80" 
                                                data-room-id="{{ $room->id }}" data-metric="pressure"
                                                data-min="{{ $room->limits['press_min'] }}" data-max="{{ $room->limits['press_max'] }}"></canvas>
                                        <div class="text-muted font-monospace mt-1" style="font-size: 0.55rem; opacity: 0.8;">
                                ...
                                @if($hasActive)
                                    <div class="active-record-list mt-3">
                                        <div class="small text-muted fw-bold mb-2 text-uppercase" style="font-size: 0.7rem;"><i class="fas fa-tasks me-1"></i> Hồ sơ đang sản xuất:</div>
                                        @foreach($room->active_records as $rec)
                                            <div class="active-record-item p-3 mb-2 rounded-3 border-start border-4 border-success bg-soft-success">
                                                <div class="d-flex justify-content-between align-items-center mb-1">
                                                    <span class="fw-bold text-navy small"><i class="fas fa-barcode me-1 text-success"></i> Số lô: {{ $rec->batch_number }}</span>
                                                    <span class="badge bg-success text-white" style="font-size: 0.65rem;">Đang ghi</span>
                                                </div>
                                                <p class="mb-2 text-muted text-truncate fw-medium" style="font-size: 0.75rem;" title="{{ $rec->product_name }}">{{ $rec->product_name }}</p>
                                ...
                                @else
                                    <div class="text-center py-3 border border-dashed rounded-3 mt-3 bg-light">
                                        @if($room->id % 7 == 0)
                                            <i class="fas fa-tools text-danger fa-lg mb-2"></i>
                                            <div class="small text-danger fw-bold">BẢO TRÌ THIẾT BỊ ĐỊNH KỲ</div>
                                            <div class="text-muted" style="font-size: 0.7rem;">Máy móc đang bảo trì định chuẩn.</div>
                                        @elseif($room->id % 5 == 0)
                                            <i class="fas fa-soap text-warning fa-lg mb-2 text-warning"></i>
                                            <div class="small text-warning fw-bold">ĐANG THỰC HIỆN VỆ SINH</div>
                                            <div class="text-muted" style="font-size: 0.7rem;">Phun cồn, khử trùng phòng cấp D.</div>
                                        @else
                                            <i class="fas fa-check-circle text-info fa-lg mb-2 text-info"></i>
                                            <div class="small text-info fw-bold">PHÒNG TRỐNG SẴN SÀNG</div>
                                            <div class="text-muted" style="font-size: 0.7rem;">Đủ điều kiện tiếp nhận lô mới.</div>
                                        @endif
                                    </div>
                                @endif
                            </div>
...
<style>
    ...
    /* Telemetry styles */
    .telemetry-panel { border: 1px solid rgba(0, 0, 0, 0.05); }
    ...
    .gradient-blue { background: linear-gradient(135deg, #0d47a1, #1976d2); }
    .gradient-teal { background: linear-gradient(135deg, #004d40, #00796b); }
    .gradient-purple { background: linear-gradient(135deg, #4a148c, #7b1fa2); }
    .gradient-orange { background: linear-gradient(135deg, #e65100, #f57c00); }
    .gradient-rose { background: linear-gradient(135deg, #880e4f, #c2185b); }
    .gradient-slate { background: linear-gradient(135deg, #263238, #455a64); }
    .gradient-success { background: linear-gradient(135deg, #1b5e20, #2e7d32); }
    .gradient-secondary { background: linear-gradient(135deg, #424242, #616161); }
            ...
            success: function(response) {
                if (response.success && response.data) {
                    ...
                    Object.keys(data).forEach(function(roomId) {
                        ...
                        checkRoomLimits(roomId, roomData.temperature, roomData.humidity, roomData.pressure);
                    });
                }
            },
