<script>
    ...
    window.openVariableSummaryModal = function() {
        ...
        if (sortedFields.length === 0) {
            body.innerHTML = '<tr><td colspan="7" class="text-center py-4 text-muted">Chưa có biến số nào được cài đặt.</td></tr>';
        }
        ...
        sortedFields.forEach(field => {
            ...
            const impText = importantVar ? `<span class="badge bg-warning text-dark">${importantVar.name}</span>` : '<span class="text-muted">-</span>';
            
            // Định dạng chi tiết cấu hình tùy theo loại biến
            let details = '';
            if (field.type === 'number') {
                details = `Min: ${field.validation.min || '-'} | Max: ${field.validation.max || '-'} | Dec: ${field.validation.decimal_places || '0'}`;
            } else if (field.type === 'formula') {
                details = `<code class="small text-primary">${field.formula || ''}</code> (Round: ${field.validation.decimal_places || '2'})`;
            } else if (field.type === 'select') {
                const dsType = (field.dataSource && field.dataSource.type) || 'manual';
                details = dsType === 'database' ? `<i class="fas fa-database me-1"></i> DB: ${field.dataSource.table}` : `<i class="fas fa-list me-1"></i> Manual: ${Array.isArray(field.options) ? field.options.length : 0} items`;
            } else {
                details = field.validation && field.validation.required ? '<span class="text-danger small"><i class="fas fa-asterisk me-1"></i>Bắt buộc</span>' : '<span class="text-muted small">Tùy chọn</span>';
            }

            tr.innerHTML = `
                <td class="text-center py-2">${index++}</td>
                ${window.isAdmin ? `<td class="font-monospace small py-2">${field.name}</td>` : ''}
