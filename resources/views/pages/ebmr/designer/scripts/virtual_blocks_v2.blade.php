@php
    // Hàng cuối bảng HEADER: cột trái "Số lô", cột phải con dấu (ban hành) — chỉ có khi
    // đang mở 1 HỒ SƠ LÔ cụ thể (thực thi/xem), không có ở chế độ Thiết kế template.
    $__hdrBatchNumber = !empty($isExecutionMode) && !empty($record->batch_number) ? $record->batch_number : null;
    $__hdrSealsHtml = '';
    if (!empty($isExecutionMode) && !empty($recordSeals)) {
        foreach ($recordSeals as $s) {
            if (empty($s->content)) {
                continue;
            }
            $borderClass = ($s->border_style ?? 'double') === 'double' ? ' seal-border-double' : '';
            $color = $s->color ?? '#dc3545';
            $fontSize = round(0.85 * ((int) ($s->size ?? 100) / 100), 3);
            $__hdrSealsHtml .=
                '<div class="v2-seal-stamp' .
                $borderClass .
                '" style="color:' .
                e($color) .
                '; font-size:' .
                ...
                'rem;">';
            if (!empty($s->header)) {
                $__hdrSealsHtml .= '<div class="seal-line-header">' . e($s->header) . '</div>';
            }
            $__hdrSealsHtml .= '<div class="seal-line-content">' . e($s->content) . '</div>';
            if (!empty($s->footer)) {
                $__hdrSealsHtml .= '<div class="seal-line-footer">' . e($s->footer) . '</div>';
            }
            $__hdrSealsHtml .= '</div>';
        }
    }
...
<script>
    (function() {
        ...
        // Mở hồ sơ theo 1 THÀNH PHẦN (?section=...): chỉ giữ block ảo HEADER, bỏ PHÊ DUYỆT /
        // THÔNG TIN SẢN PHẨM / CÔNG THỨC PHA CHẾ (chỉ có nghĩa khi xem toàn hồ sơ).
        const activeSectionId = @json($activeSectionId ?? null);
        const HEADER_ONLY_IDS = ['sys_gf_sec_header', 'sys_gf_tbl_header', 'sys_bmr_sec_header',
            'sys_bmr_tbl_header'
        ];

        // Số lô + con dấu ban hành — hàng cuối bảng HEADER (chỉ có khi mở 1 hồ sơ lô cụ thể)
        const HDR_BATCH_NUMBER = @json($__hdrBatchNumber);
        const HDR_SEALS_HTML = @json($__hdrSealsHtml);

        function buildHeaderBatchRowV2(colCount) {
            if (!HDR_BATCH_NUMBER && !HDR_SEALS_HTML) return null;
            // Chia 4 cột riêng biệt (cs:1 mỗi ô) ĐÚNG như các hàng khác của bảng: cột 1 = nhãn
            // "Số lô", cột 2 = giá trị, cột 3 = nhãn "Con dấu", cột cuối = con dấu (hoặc "NA"
            // nếu lô này không đóng dấu).
            const row = [];
            ...
            if (HDR_BATCH_NUMBER) {
                row[0].content = '<span style="font-size: 0.85rem;">Số lô</span>';
                if (colCount > 1) row[1].content = '<strong style="color:#dc3545;">: ' + HDR_BATCH_NUMBER +
                    '</strong>';
            }
            if (colCount > 1) {
                row[colCount - 2].content = '<span style="font-size: 0.85rem;">Con dấu</span>';
                row[colCount - 1].content = HDR_SEALS_HTML ?
                    '<div class="v2-seal-stamps-row">' + HDR_SEALS_HTML + '</div>' :
                    '<strong>: NA</strong>';
            }
            return row;
        }
        ...
        function generateDefaultBmrHeader() {
            ...
            if (type === 'MF') {
                ...
                const mfBatchRow = buildHeaderBatchRowV2(4);
                if (mfBatchRow) data.push(mfBatchRow);
            ...
            let data = [
                ...
                [{
                    ...
                    {
                        content: '<strong>: ' + String(t.edition).padStart(2, '0') + '</strong>',
                        rs: 1,
                        cs: 1
                    },
                    {
                ...
                [{
                    ...
                    {
                        content: '<strong>: ' + (t.pre_version !== '' ? String(t.pre_version).padStart(2, '0') :
                                '00') +
                            '</strong>',
                        rs: 1,
                        cs: 1
            ...
            const batchRow = buildHeaderBatchRowV2(4);
            if (batchRow) data.push(batchRow);
        ...
        function formatVersionHistoryDate(dateStr) {
        ...
        function generateVersionHistoryTable() {
            const versions = @json($template->version_history ?? []);
            if (!versions || versions.length === 0) return null;

            const id = 'blk_verhist_' + Date.now();
            ...
            versions.forEach((v, idx) => {
                let reasonHtml = v.change_reason || '';
                if (!reasonHtml && idx === 0) {
                    reasonHtml = '<p>Soạn thảo ấn bản mới.</p>';
                }
                data.push([{
                        content: String(v.version).padStart(2, '0'),
                        rs: 1,
                        cs: 1,
                        textAlign: 'center',
                        fontWeight: 'bold'
                    ...
                    {
                        content: formatVersionHistoryDate(v.effective_date),
                        rs: 1,
                        cs: 1,
                        textAlign: 'center'
                    }
        ...
        function generateRecipeTables() {
            ...
            [0, 1].forEach(targetType => {
                ...
                if (targetType === 1 && typeFormulas.length === 0)
            return; // Bỏ qua nếu nhóm 1 không có dữ liệu

                const tableId = 'blk_recipe_' + targetType + '_' + Date.now() + Math.floor(Math.random() *
                    1000);
                ...
                typeFormulas.forEach(formula => {
                    ...
                    if (formula.sub_amounts && formula.sub_amounts.length > 0) {
                        ...
                        formula.sub_amounts.forEach((sub, sIdx) => {
                            let subLabel = sIdx === 0 ? 'a' : (sIdx === 1 ? 'b' : (sIdx ===
                                2 ?
                        ...
                        if (formula.number_of_lots > 1 && formula.amounts_of_lots > 0) {
                            ...
                            batchText =
                                `${Number(batchVal).toLocaleString('vi-VN', {minimumFractionDigits: 2, maximumFractionDigits: 2})} = ${Number(amountOfLotsVal).toLocaleString('vi-VN', {minimumFractionDigits: 2, maximumFractionDigits: 4})} x ${formula.number_of_lots}`;
                        } else if (batchVal > 0 && t.batch_qty > 0 && unitVal > 0 && t
                            .raw_batch_size > 0) {
                    ...
                    if (typeof cellNotesForm === 'string') {
                        try {
                            cellNotesForm = JSON.parse(cellNotesForm);
                        } catch (e) {
                            cellNotesForm = {};
                        }
                    }
                    if (cellNotesForm.total_amount_per_unit) {
                        ...
                        notesHtml +=
                            `<div class="mb-1"><i class="fas fa-info-circle me-1" style="color: #17a2b8;"></i> <strong style="color: #17a2b8;">(*${cellNoteCounter}):</strong> ${cellNotesForm.total_amount_per_unit.replace(/\n/g, '<br>')}</div>`;
                    }
                    if (cellNotesForm.total_amount_per_batch) {
                        cellNoteCounter++;
                        batchText += `<sup>(*${cellNoteCounter})</sup>`;
                        notesHtml +=
                            `<div class="mb-1"><i class="fas fa-info-circle me-1" style="color: #17a2b8;"></i> <strong style="color: #17a2b8;">(*${cellNoteCounter}):</strong> ${cellNotesForm.total_amount_per_batch.replace(/\n/g, '<br>')}</div>`;
                    }
                    ...
                    materials.forEach((mat, mIdx) => {
                        ...
                        if (typeof cellNotesMat === 'string') {
                            try {
                                cellNotesMat = JSON.parse(cellNotesMat);
                            } catch (e) {
                                cellNotesMat = {};
                            }
                        }
                        ...
                        if (cellNotesMat.code) {
                            ...
                            notesHtml +=
                                `<div class="mb-1"><i class="fas fa-info-circle me-1" style="color: #17a2b8;"></i> <strong style="color: #17a2b8;">(*${cellNoteCounter}):</strong> ${cellNotesMat.code.replace(/\n/g, '<br>')}</div>`;
                        }
                        ...
                        if (cellNotesMat.name) {
                            ...
                            notesHtml +=
                                `<div class="mb-1"><i class="fas fa-info-circle me-1" style="color: #17a2b8;"></i> <strong style="color: #17a2b8;">(*${cellNoteCounter}):</strong> ${cellNotesMat.name.replace(/\n/g, '<br>')}</div>`;
                        }
                        ...
                        if (cellNotesMat.manufacturer) {
                            ...
                            notesHtml +=
                                `<div class="mb-1"><i class="fas fa-info-circle me-1" style="color: #17a2b8;"></i> <strong style="color: #17a2b8;">(*${cellNoteCounter}):</strong> ${cellNotesMat.manufacturer.replace(/\n/g, '<br>')}</div>`;
                        }
                        ...
                        if (cellNotesMat.Spec) {
                            ...
                            notesHtml +=
                                `<div class="mb-1"><i class="fas fa-info-circle me-1" style="color: #17a2b8;"></i> <strong style="color: #17a2b8;">(*${cellNoteCounter}):</strong> ${cellNotesMat.Spec.replace(/\n/g, '<br>')}</div>`;
                        }
                ...
                if (notesHtml) {
                    resultBlocks.push({
                        id: 'blk_notes_' + targetType + '_' + Date.now() + Math.floor(Math
                        .random() * 1000),
                        type: 'static-text',
        ...
        function generateSignatureTable() {
            ...
            // 1. Soạn thảo
            let authorSigContent =
                `<span class="text-success"><i class="fas fa-check-circle me-1"></i> Đã ký</span>`;
        ...
        window.__V2_buildVirtualBlocks = function() {
            ...
            if (activeSectionId) {
                virtualBlocks = virtualBlocks.filter(function(b) {
                    return HEADER_ONLY_IDS.includes(b.id);
                });
            }

            // Mọi block ảo đều bị khóa (không cho sửa trong V2)
            virtualBlocks.forEach(function(b) {
                b.locked = true;
            });
        ...
        window.__V2_sysLabels = ['BMR HEADER', 'BMR Header', 'GF Header', 'QUY TRÌNH PHÊ DUYỆT',
            'THÔNG TIN CHUNG SẢN PHẨM',
            'MÔ TẢ SẢN PHẨM', 'CÔNG THỨC PHA CHẾ', '1. NGUYÊN LIỆU PHA CHẾ',
            '2. NGUYÊN LIỆU KHÁC (BAO PHIM/NANG)',
            'Ghi chú nguyên liệu pha chế', 'Ghi chú nguyên liệu khác', 'THÔNG TIN CHUNG', 'Trình Ký'
        ];
        if ("{{ $template->type ?? 'BMR' }}" === "GF") {
            window.__V2_sysLabels = [];
        }
    })();
</script>
