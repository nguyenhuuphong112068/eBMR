    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content border-0 shadow-lg" style="border-radius: 12px;">
            <div class="modal-header bg-light border-0 py-3 px-4">
                <h5 class="modal-title fw-bold text-success" id="assign_room_title"><i class="fas fa-desktop me-1"></i> Khai báo thiết bị trong phòng</h5>
                ...
                <div class="row align-items-end mb-4 p-3 border rounded bg-light">
                    <div class="col-md-9">
                        <div class="form-group mb-0">
                            <label class="fw-bold small text-uppercase text-muted mb-2"><i class="fas fa-plug me-1"></i> Chọn thiết bị cố định để thêm</label>
                            <select class="form-control" id="assign_equipment_id">
                                <option value="">-- Đang tải thiết bị --</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <button type="button" class="btn btn-success w-100 fw-bold d-flex align-items-center justify-content-center" id="btn_assign_confirm" style="height: 38px;">
...
$(document).ready(function() {
    ...
    window.loadRoomEquipments = function(roomId) {
        $('#assigned_equipments_body').html('<tr><td colspan="5" class="text-center text-muted py-3"><i class="fas fa-spinner fa-spin me-2"></i> Đang tải danh sách thiết bị...</td></tr>');
        $('#assign_equipment_id').html('<option value="">-- Đang tải thiết bị --</option>');
        assignedIds = [];
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if(response.assigned && response.assigned.length > 0) {
                    ...
                    html = '<tr><td colspan="5" class="text-center text-muted py-3">Chưa có thiết bị cố định nào được khai báo trong phòng này.</td></tr>';
                }
                $('#assigned_equipments_body').html(html);

                // 2. Điền dữ liệu vào dropdown (chỉ hiển thị những thiết bị chưa gán)
                let selectHtml = '<option value="">-- Chọn thiết bị để thêm --</option>';
                if(response.allFixed && response.allFixed.length > 0) {
                    ...
                    response.allFixed.forEach(function(item) {
                        if (!assignedIds.includes(item.id)) {
                            hasAvailable = true;
                            selectHtml += `<option value="${item.id}">${item.code} - ${item.name}</option>`;
                        }
                    });
                    if(!hasAvailable) {
                        selectHtml = '<option value="">-- Tất cả thiết bị cố định đã được khai báo --</option>';
                    }
                } else {
                    selectHtml = '<option value="">-- Không tìm thấy thiết bị cố định nào trong hệ thống --</option>';
                }
                $('#assign_equipment_id').html(selectHtml);
            },
            error: function() {
                $('#assigned_equipments_body').html('<tr><td colspan="5" class="text-center text-danger py-3">Lỗi khi tải danh sách thiết bị. Vui lòng thử lại.</td></tr>');
            }
        });
    };
    ...
    $('#btn_assign_confirm').click(function() {
        ...
        const equipmentId = $('#assign_equipment_id').val();

        if(!equipmentId) {
            Swal.fire({
                title: 'Cảnh báo!',
                text: 'Vui lòng chọn một thiết bị.',
                icon: 'warning',
                timer: 2000,
