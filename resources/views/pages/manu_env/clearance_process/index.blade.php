...
@section('mainContent')
    <style>
        ...
        #clearance-process-table .note-editor.note-frame {
            border: none !important;
            border-radius: 0 !important;
            margin-bottom: 0 !important;
        }

        #clearance-process-table .note-editor .note-toolbar {
            border-radius: 0 !important;
            border-bottom: 1px solid #dee2e6;
            background-color: #f8f9fa;
        }

        #clearance-process-table .note-editor .note-statusbar {
            border-radius: 0 !important;
        }

        /* Bỏ padding của ô chứa editor và ô tiêu chuẩn */
        #clearance-process-table td.editor-cell,
        #clearance-process-table td.standard-cell {
            padding: 0 !important;
        }

        /* Xóa viền và bo góc của textarea tiêu chuẩn */
        #clearance-process-table .step-standard {
            border: none !important;
            border-radius: 0 !important;
            ...
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-12 d-flex justify-content-between align-items-center">
                        <h1 class="m-0 text-uppercase fw-bold text-navy">Thiết kế Quy trình dọn quang - {{ $entityCode }}
                            ({{ $entityName }}) - Ấn bản V.{{ $list->version }}</h1>
                        <div>
                            <a href="{{ route('pages.manu_env.clearance_process.list', ['type' => $type, 'id' => $id]) }}"
            ...
            <div class="container-fluid">
                <div class="card shadow-sm border-0">
                    <div class="card-body p-0">
                        <table class="table table-bordered mb-0" id="clearance-process-table">
    ...
    <script>
        $(document).ready(function() {
            ...
            const uploadImage = function(file, editor) {
                ...
                $.ajax({
                    url: '{{ route('pages.manu_env.clearance_process.upload_image') }}',
                    method: 'POST',
                    headers: {
            ...
            $('#btn-save-process, #btn-save-process-bottom').click(function() {
                ...
                $.ajax({
                    url: '{{ route('pages.manu_env.clearance_process.store', ['type' => $type, 'list_id' => $list_id]) }}',
                    method: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        processes: processes
