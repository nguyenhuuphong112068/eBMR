...
@section('mainContent')
    ...
    @section('script')
        <script>
            ...
            const EDIT_STEP_URL_BASE = "{{ url('/manu_env/clearance-process/campaign') }}";
            const STEP_HISTORY_URL_BASE = "{{ url('/manu_env/clearance-process/campaign/step') }}";
            ...
            function finishCampaign() {
                Swal.fire({
                    ...
                    $.ajax({
                        ...
                        success: function(res) {
                            if (res.success) {
                                // Nếu campaign gắn với 1 phiên sản xuất (distribution_id), server trả
                                // sẵn redirect_url vào thẳng trang ghi chép hồ sơ — dọn quang đã xong
                                // là điều kiện cuối cùng mở khóa trang này (xem completeCampaign()).
                                const nextUrl = res.redirect_url || '{{ route('pages.ebmr.production') }}';
                                Swal.fire({
                                    icon: 'success',
                                    title: 'Hoàn thành!',
                                    text: res.message,
