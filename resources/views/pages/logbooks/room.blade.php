    ...
    <div class="content-wrapper">
        <div class="container-fluid py-4">
            ...
            <div class="d-flex align-items-center mb-4">
                ...
                <div>
                    <h3 class="mb-0 text-navy fw-bold">SỔ NHẬT KÝ PHÒNG: {{ $room->code }}</h3>
                    <p class="text-muted mb-0 small">Tên phòng: {{ $room->name ?? $room->code }}</p>
                </div>
            </div>
                            ...
                            <tbody>
                                @forelse($logbooks as $log)
                                    <tr>
                                        ...
                                        <td>
                                            <div class="small fw-bold text-navy">{{ $log->start_time->format('d/m/Y H:i') }}
                                            </div>
                                            <div class="small text-muted">đến
                                                {{ $log->end_time ? $log->end_time->format('d/m/Y H:i') : 'Đang tiếp tục' }}
                                            </div>
                                        </td>
                                        <td>
                                            @php
                                                ...
                                                $style = $actionStyles[$log->action_type] ?? [
                                                    'color' => 'secondary',
                                                    'icon' => 'fa-question',
                                        ...
                                        <td>
                                            @if ($log->product_name)
                                                ...
                                                <div class="small text-muted font-monospace"><i class="fas fa-barcode"></i>
                                                    {{ $log->batch_number }}</div>
                                            @else
                                        ...
                                        <td>
                                            <span
                                                class="small fw-bold {{ $log->clean_level ? 'text-primary' : 'text-muted' }}">
                                                {{ $log->clean_level ?: '-' }}
                                            </span>
                                        </td>
                                        <td>
                                            <span
                                                class="small {{ $log->clean_expiry_date ? 'text-success fw-bold' : 'text-muted' }}">
                                        ...
                                        <td class="text-center">
                                            <div class="d-flex align-items-center justify-content-center gap-2">
                                                <span
                                                    class="badge bg-light text-muted border">{{ ucfirst($log->previous_status) }}</span>
                                                <i class="fas fa-arrow-right text-muted small"></i>
                                                <span
                                                    class="badge bg-light text-navy border border-primary">{{ $log->current_status ? ucfirst($log->current_status) : 'Đang xử lý' }}</span>
                                            </div>
                                        </td>
                                        <td class="text-center">
                                            @php
                                                $emps = is_array($log->employee_ids)
                                                    ? $log->employee_ids
                                                    : json_decode($log->employee_ids, true);
                                                $empNames = [];
                                                if (is_array($emps)) {
                                        ...
                                        <td class="text-center">
                                            @if ($log->action_type === 'cleaning' && $log->campaign_id)
                                                <a href="{{ route('pages.manu_env.cleaning_process.campaign.open', ['room_id' => $log->room_id]) }}?campaign_id={{ $log->campaign_id }}"
                                                class="btn btn-sm btn-outline-primary rounded-pill px-3 shadow-sm"
                                                target="_blank" data-bs-toggle="tooltip" title="Xem hồ sơ vệ sinh">
                                                <i class="fas fa-eye me-1"></i> Xem HS
                                                </a>
                                                <a href="{{ route('pages.manu_env.cleaning_process.campaign.print', ['room_id' => $log->room_id]) }}?campaign_id={{ $log->campaign_id }}"
                                                    class="btn btn-sm btn-outline-danger rounded-pill px-3 shadow-sm ms-1"
                                                    target="_blank" data-bs-toggle="tooltip" title="In báo cáo PDF">
